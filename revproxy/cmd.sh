...
# - RPS_HOSTNAME
# - LAULUD_HOSTNAME
# - OSRS_HISCORE_PROXY_HOSTNAME
# - AMPLIFY_API_KEY_FILE

set -e
echo "Filling site template confs..."
envsubst '${ROOT_HOSTNAME}' < nginx.conf > /etc/nginx/nginx.conf
(cd sites; for f in *; do envsubst '${ROOT_HOSTNAME} ${RPS_HOSTNAME} ${LAULUD_HOSTNAME} ${OSRS_HISCORE_PROXY_HOSTNAME}' < $f > /etc/nginx/conf.d/$f; done)
